...
ssppath="/project/cmip5/rcp85/Amon/"

histpath_ps="/project/cas/islas/CMIP5/download/ps/hist/"
ssppath_ps="/project/cas/islas/CMIP5/download/ps/rcp85/"

pathout="/project/cas/islas/python_savs/qtrend_paper/DATA_SORT/CMIP5/vapor_pressures/"

cmip5models=pd.read_csv('cmip5csvinfo.csv')

ybegp = 1980 ; monbegp = 1 ; yendp = 2005 ; monendp = 12 # dates for Past period
ybegf = 2006 ; monbegf = 1 ; yendf = 2021 ; monendf = 12 # dates for Future period
...
for index, modname in models.iteritems():
    ...
    for imem in range(1, nmems+1,1):
        ...
        histdir_ps = glob(histpath_ps+"/"+modname+"/"+memstr)
        histdir_ps = histdir_ps[0]

        sspdir_ps = glob(ssppath_ps+"/"+modname+"/"+memstr)
        ...
        if (modname == 'MIROC5'):
            ps_hist = ps_hist.sel(time=slice("1980-01-01","2005-12-31"))
            ps_ssp = ps_ssp.sel(time=slice("2006-01-01","2021-12-31"))
