...
pathout="/project/cas/islas/python_savs/qtrend_paper/DATA_SORT/CMIP6/vapor_pressures_climps/"

cmip6models=pd.read_csv('cmip6csvinfo.csv')

ybegp = 1980 ; monbegp = 1 ; yendp = 2014 ; monendp = 12 # dates for Past period
ybegf = 2015 ; monbegf = 1 ; yendf = 2020 ; monendf = 12 # dates for Future period
...
for index, modname in models.iteritems():
    ...
    for imem in range(1, nmems+1,1):
        ...
        if (modname == 'EC-Earth3'):
            ps['time'] = pd.date_range("1980-01","2021-01",freq='M') 
            huss['time'] = pd.date_range("1980-01","2021-01", freq='M')
            tas['time'] = pd.date_range("1980-01","2021-01", freq='M')


        psclim = ps.ps.groupby('time.month').mean('time')
