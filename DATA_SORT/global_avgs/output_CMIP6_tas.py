...
for index, modname in models.iteritems():
    ...
    dat = xr.open_dataset(datpath+"vaporpressures_"+modname+".nc").tas
    dat = dat.sel(time=slice("1980-01","2020-12"))
    dat['time'] = timenew
    dat['lon'] = landmask.lon ; dat['lat'] = landmask.lat


    dat_all = avg.cosweightlonlat(dat, 0, 360, -90, 90)
    dat_ocean = avg.cosweightlonlat(dat*oceanfrac, 0, 360, -90, 90)
    dat_ocean_60sn = avg.cosweightlonlat(dat*oceanfrac, 0, 360, -60, 60)
    ...
    dat_all = dat_all.rename('global')
    dat_ocean = dat_ocean.rename('ocean')
    dat_ocean_60sn = dat_ocean_60sn.rename('ocean_60sn')
    ...
    dat = xr.merge([dat_all, dat_ocean, dat_ocean_60sn, dat_land, dat_arid, dat_humid, dat_hyperarid])
...
cmip6.to_netcdf(pathout+'CMIP6_tas.nc')
