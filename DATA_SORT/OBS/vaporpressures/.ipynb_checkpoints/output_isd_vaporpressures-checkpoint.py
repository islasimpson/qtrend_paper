...
isd = xr.open_dataset("/project/cas/islas/python_savs/qtrend_paper/DATA_SORT/ISD/1980_2020/T2M_DEW_ISD_global_1980_2020.nc", use_cftime=True)

#----drop the lons and lats and add them back in later
lon = isd.lon
lat = isd.lat
isd = isd.drop(['lon','lat'])


#----Check for the number of dew point NaN's in a month
#----drop stations that have more than 10 nan days in amonth
...
isd_monthly = xr.merge([isd_monthly, lon, lat])
#--------
...
newlons = xr.where(isd_monthly['lon'] < 0, isd_monthly['lon'] + 360., isd_monthly['lon'])
isd_monthly['lon'] = newlons
#--------
...
for istation in np.arange(0,isd_monthly.station.size,1):
    print(istation)
    lonval = isd_monthly.lon.isel(station=istation).values
    latval = isd_monthly.lat.isel(station=istation).values
...
qrel.to_netcdf(pathout+'vaporpressures_ISD.nc')
svp.to_netcdf(pathout+'vaporpressures_ISD.nc', mode='a')
vp.to_netcdf(pathout+'vaporpressures_ISD.nc', mode='a')
vpd.to_netcdf(pathout+'vaporpressures_ISD.nc', mode='a')
lon.to_netcdf(pathout+'vaporpressures_ISD.nc', mode='a')
lat.to_netcdf(pathout+'vaporpressures_ISD.nc', mode='a')
nbaddays.to_netcdf(pathout+'vaporpressures_ISD.nc', mode='a')
