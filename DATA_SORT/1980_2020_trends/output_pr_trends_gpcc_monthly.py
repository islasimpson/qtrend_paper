...
dat = xr.open_dataset("/project/cas/islas/python_savs/qtrend_paper/DATA_SORT/OBS/precip/"+
 "GPCC.nc")
dat = dat.sel(time=slice("1980-01-01","2020-12-31")).pr
...
dat_monthly = xr.DataArray(dat_monthly, dims=['year', 'month','lat','lon'],
                coords=[yearvals, np.arange(1,13,1), lat, lon], name='pr')
...
dat_monthly_trend.to_netcdf(pathout+'prtrends_GPCC_monthly.nc')
